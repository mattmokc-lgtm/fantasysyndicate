...
def load_teams():
    q = """SELECT team_id,  location||team_name as team_name, division, logo_url, banner_url, 
            ...
            fantrax_url FROM teams where team_id = 8 ORDER BY team_name"""
    return read_sql_df(q)

@st.cache_data(ttl=60)
def load_team_by_team_id(team_id):
    q = "SELECT * FROM teams WHERE team_id = '8' LIMIT 1"
    df = read_sql_df(q)
...
# Build object key
object_key = f"{R2_DIR}/LA update.png"
...
def page_team_dashboard(team_id):
    ...
    def format_dead(value, default='—'):
        ...
        if value is None or (isinstance(value, (int, float)) and value == 0):
            return default
        return f"${value:,.2f}"
    left_html = f'<div class="team-box"><h3 style="color:{team["secondary_color"]}">Roster</h3></div>'
