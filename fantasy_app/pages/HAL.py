...
def load_teams():
    q = """SELECT team_id,  location||team_name as team_name, division, logo_url, banner_url, 
            ...
            fantrax_url FROM teams where team_id = 7 ORDER BY team_name"""
    return read_sql_df(q)

@st.cache_data(ttl=60)
def load_team_by_team_id(team_id):
    q = "SELECT * FROM teams WHERE team_id = '7' LIMIT 1"
    df = read_sql_df(q)
...
# Build object key
object_key = f"{R2_DIR}/HAL.png"

def render_banner(team):
    primary = "#000000" or "#2d2d2d"
    try:
