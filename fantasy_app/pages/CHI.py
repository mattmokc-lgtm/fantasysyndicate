...
def load_teams():
    q = """SELECT team_id,  location||team_name as team_name, division, logo_url, banner_url, 
            ...
            fantrax_url FROM teams where team_id = 4 ORDER BY team_name"""
    return read_sql_df(q)

@st.cache_data(ttl=60)
def load_team_by_team_id(team_id):
    q = "SELECT * FROM teams WHERE team_id = '4' LIMIT 1"
    df = read_sql_df(q)
...
def page_team_dashboard(team_id):
    ...
    <div style="padding:12px;border-radius:8px;background:rgba(255,255,255,0.08);text-align:center;color: #000000;">
    ...
    if df.empty:
        ...
        right_html = (
            '<div style="display:flex;gap:10px;align-items:center;">'
            f'<div style="padding:12px;border-radius:8px;background:rgba(255,255,255,0.08);text-align:center;display:inline-block;;color: #000000;">'
