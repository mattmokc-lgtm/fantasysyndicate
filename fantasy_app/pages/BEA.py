...
def load_teams():
    q = "SELECT team_id,  location||team_name as team_name, division, logo_url, banner_url, primary_color, secondary_color, accent_color, neutral_color, additional_color, fantrax_url FROM teams where team_id = 2 ORDER BY team_name"
    return read_sql_df(q)

@st.cache_data(ttl=60)
def load_team_by_team_id(team_id):
    q = "SELECT * FROM teams WHERE team_id = '2' LIMIT 1"
    df = read_sql_df(q)
...
# Build object key
object_key = f"{R2_DIR}/beamsville.png"

def render_banner(team):
    primary = team.get("primary_color") or "#2d2d2d"
    try:
    ...
    st.markdown(
        f"""
    <div style="padding:18px;border-radius:10px;background:{primary};color:{text_color};display:flex;justify-content:space-between;align-items:center;border: 2px solid {team['neutral_color']};">
      {left_html}
      {right_html}
    </div>
...
def page_team_dashboard(team_id):
    ...
    st.markdown(f"""
                <style>
                    ...
                    [data-testid="stDataFrame"] .ag-row:hover .ag-cell {{
                        background-color: {team['neutral_color']}22 !important;
                    }}
                    ...
                    .team-box {{
                        ...
                        padding: 1rem;
                        border-radius: 3px;
                        /* REMOVED: border: 2px solid {team['neutral_color']}; */
    ...
    st.markdown(
        ...
        <div style="
            ...
            border: 2px solid {team['neutral_color']}; 
            margin-bottom: 1rem;
        ">
        {left_html}
        {right_html}
    ...
    border: 2px solid {team['neutral_color']}; 
    margin-bottom: 1rem;
    "><h3 style="color:{team["primary_color"]}">Trade History</h3></div>""", unsafe_allow_html=True)
    st.dataframe(
        trades_display,
        width='stretch',
        height=420
    ...
    st.markdown(f"""    <div style="
        ...
        border: 2px solid {team['neutral_color']}; 
        margin-bottom: 1rem;
        "><h3 style="color:{team["primary_color"]}">Awards</h3></div>""", unsafe_allow_html=True)
    if trophies.empty:
