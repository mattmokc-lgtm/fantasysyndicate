...
def load_teams():
    q = """SELECT team_id,  location||team_name as team_name, division, logo_url, banner_url, 
            ...
            fantrax_url FROM teams where team_id = 10 ORDER BY team_name"""
    return read_sql_df(q)

@st.cache_data(ttl=60)
def load_team_by_team_id(team_id):
    q = "SELECT * FROM teams WHERE team_id = '10' LIMIT 1"
    df = read_sql_df(q)
...
def render_banner(team):
    ...
    st.markdown(
        f"""
    <div style="padding:18px;border-radius:10px;background:{primary};color:{text_color};display:flex;justify-content:space-between;align-items:center;border: 2px solid {team['secondary_color']}; ">
      {left_html}
      {right_html}
    </div>
    ...
    st.markdown(f"""
            <style>
                ...
                .team-box {{
                    ...
                    padding: 1rem;
                    border-radius: 3px;
                    /* REMOVED: border: 2px solid {team['secondary_color']}; */
