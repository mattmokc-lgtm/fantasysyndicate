...
def load_teams():
    q = "SELECT team_id,  location||team_name as team_name, division, logo_url, banner_url, primary_color, secondary_color, accent_color, neutral_color, additional_color, fantrax_url FROM teams where team_id = 1 ORDER BY team_name"
    return read_sql_df(q)

@st.cache_data(ttl=60)
def load_team_by_team_id(team_id):
    q = "SELECT * FROM teams WHERE team_id = '1' LIMIT 1"
    df = read_sql_df(q)
