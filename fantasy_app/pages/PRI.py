...
def load_teams():
    q = """SELECT team_id,  location||team_name as team_name, division, logo_url, banner_url, 
            ...
            fantrax_url FROM teams where team_id = 11 ORDER BY team_name"""
    return read_sql_df(q)

@st.cache_data(ttl=60)
def load_team_by_team_id(team_id):
    q = "SELECT * FROM teams WHERE team_id = '11' LIMIT 1"
    df = read_sql_df(q)
...
# Build object key
object_key = f"{R2_DIR}/PRI fixed.png"

def render_banner(team):
    primary = team.get("secondary_color") or "#2d2d2d"
    try:
    ...
    <div style="display:flex;align-items:center;gap:16px;">
      ...
      <div style="width:{logo_size};height:{logo_size};border-radius:0px;background:{primary};padding:0px;">
        <!-- Increased image width/height -->
        <img src="{image_src}" style="width:{logo_size};height:{logo_size};object-fit:contain;" />
      </div>
    ...
    st.markdown(
        f"""
    <div style="padding:18px;border-radius:10px;background:{primary};color:{text_color};display:flex;justify-content:space-between;align-items:center;border: 2px solid {team['primary_color']}; ">
      {left_html}
      {right_html}
    </div>
...
def page_team_dashboard(team_id):
    ...
    st.markdown(f"""
            <style>
                ...
                [data-testid="stDataFrame"] .ag-header-cell {{
                    background-color: {team['secondary_color']} !important;
                ...
                .team-box {{
                    ...
                    padding: 1rem;
                    border-radius: 3px;
                    /* REMOVED: border: 2px solid {team['primary_color']}; */
    ...
    border: 2px solid {team['primary_color']}; 
    margin-bottom: 1rem;
    "><h3 style="color:{team["secondary_color"]}">Trade History</h3></div>""", unsafe_allow_html=True)
    st.dataframe(
        trades_display,
        width='stretch',
        height=420
    ...
    st.markdown(f"""    <div style="
        ...
        border: 2px solid {team['primary_color']}; 
        margin-bottom: 1rem;
        "><h3 style="color:{team["secondary_color"]}">Awards</h3></div>""", unsafe_allow_html=True)
    if trophies.empty:
